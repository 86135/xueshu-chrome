...
/*else*/ if(url.hostname==="trainer-pc.codemao.cn"){chrome.tabs.insertCSS(tabId,{code:"html{display:none!important;}"});return;}
...
let json=JSON.parse(xhr.responseText);if((json.board_id==="11"||json.board_id==="26")||json.board_id==="7")chrome.tabs.insertCSS(tabId,{code:"html{display:none!important;}"});return;}}
...
chrome.storage.onChanged.addListener((changes,areaName)=>{chrome.storage.local.get({xueshu:"y"},(res)=>{if(res.xueshu==="y")chrome.tabs.onUpdated.addListener(func);else chrome.tabs.onUpdated.removeListener(func);});})
//论box3与前端的关系
